...
class LabelTests(TestCase):
    ...
    def test_printer_name(self):
        ...
        self.assertEqual(
            print_server.selected_printer.verbose_name, 'Dummy Printer')

    def test_label(self):
        """Assert labels were printed and job ids returned as per number of copies requested."""
        context = {'name': 'Test Label'}
        label = Label('default', context=context, print_server=DummyPrintServer(
    ...
    def test_printer(self):
        context = {'name': 'Test Label'}
        label = Label(
            'aliquot', context=context, print_server=DummyPrintServer())
        self.assertEqual(str(label), 'dummy_printer@localhost')
        self.assertEqual(
            label.print_server.selected_printer.label, 'dummy_printer')
        self.assertEqual(
            label.print_server.selected_printer.verbose_name, 'Dummy Printer')
    ...
    def test_label_no_server(self):
        ...
        context = {'name': 'Test Label'}
        label = Label(
            'requisition', context=context, print_server=DummyPrintServer())
        self.assertIn(
            'Unable to connect to CUPS', label.print_server.error_message)
        label.print_label(1)
        self.assertIn(
            'Unable to connect to CUPS', label.print_server.error_message)
    ...
    def test_label_no_printer(self):
        ...
        label.print_label(1)
        self.assertIn(
            'Print job failed. No printer selected.', label.error_message)
    ...
    def test_label_invalid_printer(self):
        ...
        ), printer_name='invalid_printer')
        self.assertIn(
            'Printer \'invalid_printer\' not found on \'localhost\'.', label.print_server.error_message)
        label.print_label(1)
        self.assertIn(
            'Printer \'invalid_printer\' not found on \'localhost\'.', label.print_server.error_message)
